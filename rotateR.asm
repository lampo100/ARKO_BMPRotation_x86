...
# Wyrażone jest to wzorem P(x,y) = (Wysokość - y)*Szerokość + x  gdzie Wysokość i Szerokość są to wymiary źródła
...
pocz:
 mov                eax, height
 mov                ebx,[pixelx]
 sub                eax, ebx                 ;w eax jest (wysokość - y)(NIE PIXELY, POZYCJA Y W MACIERZY)
 mov                ebx, width
 mul                ebx                      ; w eax jest (Wysokość - y)*Szerokość
 mov                ebx, [pixely]
 add                eax, ebx                 ; w eax jest (Wysokość - y)*Szerokość + x
...
tutaj:
 ...
 mov                ebx, [pixelx]
 mov                eax, height
 cmp                ebx, eax                ; sprawdzamy czy doszliśmy do końca wiersza
 jne                continuex
zerox:
